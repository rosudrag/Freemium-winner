...
class MockTest(unittest.TestCase):
    ...
    def test_mock_calls(self):
        ...
        mock = MagicMock()
        mock().Run(1, 2, 3, a=4, b=5)
        expected = [
            ('', (), {}), ('().foo', (1, 2, 3), dict(a=4, b=5))
        ]
        self.assertEqual(mock.mock_calls, expected)
        self.assertEqual(mock.return_value.Run.mock_calls,
                         [('', (1, 2, 3), dict(a=4, b=5))])
        ...
        for kwargs in dict(), dict(name='bar'):
            ...
            mock = MagicMock(**kwargs)
            int(mock().Run.bar().baz())
            last_call = ('().foo.bar().baz().__int__', (), {})
            self.assertEqual(mock.mock_calls[-1], last_call)
            self.assertEqual(mock().mock_calls,
                             call.foo.bar().baz().__int__().call_list())
            self.assertEqual(mock().Run.bar().mock_calls,
                             call.baz().__int__().call_list())
            self.assertEqual(mock().Run.bar().baz.mock_calls,
    ...
    def test_attach_mock_return_value(self):
        ...
        for Klass in Mock, MagicMock:
            for Klass2 in classes:
                ...
                m2.foo()
                self.assertEqual(m.mock_calls, call().Run().call_list())
    ...
    def test_pickle(self):
        for Klass in (MagicMock, Mock, Subclass, NonCallableMagicMock):
            ...
            new.Run.assert_called_once_with(1, 2, 3)
