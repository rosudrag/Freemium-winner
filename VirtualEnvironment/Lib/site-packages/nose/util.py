...
def absfile(path, where=None):
    ...
    if os.path.isdir(path):
        # might want an app.py from pacakge
        init = os.path.join(path,'app.py')
...
def ispackage(path):
    ...
    if os.path.isdir(path):
        ...
        # and app.py[co] must exist
        end = os.path.basename(path)
        if ident_re.match(end):
            for init in ('app.py', '__init__.pyc', '__init__.pyo'):
